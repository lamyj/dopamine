...
BOOST_AUTO_TEST_CASE(AsBSONStringsUnicode)
{
    odil::DataSet data_set;
    data_set.add(
    ...
    check_bson_object(
        bson, {std::string(odil::registry::SpecificCharacterSet), "deadbeef"});
    mongo::BSONElement const element = bson.getField("deadbeef");
    BOOST_REQUIRE(element.isABSONObj());
    check_bson_object(element.Obj(), {"vr", "Value"});
    check_bson_string(element.Obj().getField("vr"), "LO");
    ...
    check_bson_string(array[0], "J\xc3\xa9r\xc3\xb4me");
}
...
BOOST_AUTO_TEST_CASE(AsBSONPersonNameUnicode)
{
    odil::DataSet data_set;
    data_set.add(
        odil::registry::SpecificCharacterSet,
        odil::Value::Strings({"ISO_IR 100"}), odil::VR::PN);
    data_set.add(
    ...
    std::vector<mongo::BSONElement> array =
            element.Obj().getField("Value").Array();
    BOOST_REQUIRE_EQUAL(array.size(), 1);
    check_bson_object(array[0].Obj(), {"Alphabetic"});
    check_bson_string(
        array[0].Obj().getField("Alphabetic"), {"Buc^J\xc3\xa9r\xc3\xb4me"});
}
...
{
    auto const bson = BSON(
        "deadbeef" << BSON(
            "vr" << "SS" << "Value" << BSON_ARRAY(
                1 << (long long)2 << (double)5 )));
...
BOOST_AUTO_TEST_CASE(AsDataSetReals)
{
    auto const bson = BSON(
        "deadbeef" << BSON(
            "vr" << "FL" << "Value" << BSON_ARRAY(12.34 << 56.78)));
...
BOOST_AUTO_TEST_CASE(AsDataSetStrings)
{
    auto const bson = BSON(
        "deadbeef" << BSON(
            "vr" << "CS" << "Value" << BSON_ARRAY("FOO" << "BAR")));
...
BOOST_AUTO_TEST_CASE(AsDataSetStringsUnicode)
{
    auto const bson = BSON(
        std::string(odil::registry::SpecificCharacterSet) << BSON(
            "vr" << "CS" << "Value" << BSON_ARRAY("ISO_IR 100"))
        << "deadbeef" << BSON(
            "vr" << "LO" << "Value" << BSON_ARRAY("J\xc3\xa9r\xc3\xb4me")));

    odil::DataSet const data_set = dopamine::as_dataset(bson);
    BOOST_REQUIRE(!data_set.empty());

    BOOST_REQUIRE(data_set.has("deadbeef"));
    BOOST_REQUIRE(data_set.get_vr("deadbeef") == odil::VR::LO);
    BOOST_REQUIRE(data_set.is_string("deadbeef"));
    BOOST_REQUIRE(data_set.as_string("deadbeef") ==
                  odil::Value::Strings({"J\xe9r\xf4me"}));
}
...
BOOST_AUTO_TEST_CASE(AsDataSetPersonName)
{
    auto const bson = BSON(
        "deadbeef" << BSON(
            "vr" << "PN" << "Value" << BSON_ARRAY(
                BSON(
                    "Alphabetic" << "Alpha^Betic" <<
...
BOOST_AUTO_TEST_CASE(AsDataSetPersonNameUnicode)
{
    auto const bson = BSON(
        std::string(odil::registry::SpecificCharacterSet) << BSON(
            "vr" << "CS" << "Value" << BSON_ARRAY("ISO_IR 100"))
        << "deadbeef" << BSON(
            "vr" << "PN" << "Value" << BSON_ARRAY(
                BSON("Alphabetic" << "Buc^J\xc3\xa9r\xc3\xb4me"))));
    ...
    BOOST_REQUIRE(data_set.is_string("deadbeef"));
    BOOST_REQUIRE(data_set.as_string("deadbeef") ==
                  odil::Value::Strings({"Buc^J\xe9r\xf4me"}));
}
...
BOOST_AUTO_TEST_CASE(AsDataSetDataSets)
{
    auto const bson = BSON(
        "deadbeef" << BSON(
            "vr" << "SQ" << "Value" << BSON_ARRAY(
                BSON(
                    "beeff00d" << BSON(
                        "vr" << "SS" << "Value" << BSON_ARRAY(1))))));
